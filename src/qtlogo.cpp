...
void QtLogo::buildGeometry(int divisions, qreal scale)
{
  ...
  RectPrism cube(geom, 1.0, 1.0, 1.0);
  
  //parts << cube.parts;//stem.parts << cross.parts << body.parts;
  Patch *fb = new Patch(geom);
  fb->addQuad(QVector3D(-0.5, 0.5, 0.), QVector3D(0.5, 0.5, 0), QVector3D(0.5, -0.5, 0.), QVector3D(-0.5, -0.5, 0.));
  parts << fb;
  geom->finalize();
}
//! [3]
